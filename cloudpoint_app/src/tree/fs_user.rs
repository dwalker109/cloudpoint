use super::CtrLeaf;
use crate::ctr_fs::fs_user::{Archive, InnerPath};
use crate::tree::CtrContext;
use anyhow::Result;
...
impl Leaf for FsUserLeaf {
    ...
    fn delete(&mut self, ctx: &Self::Context) -> Result<(), TreeError> {
        ...
        let path = InnerPath::new(&self.path)?;
        ...
        match ctx.archive.open_file(&path, FS_OPEN_READ) {
            ...
            Err(_) => {
                ...
                for sep in path_separators {
                    let dir_path = InnerPath::new(&self.path[0..=sep])?;
...
pub fn from_archive(archive: Rc<Archive>) -> Result<Tree<CtrLeaf>> {
    ...
    fn walk_sub(
        ...
        let path = InnerPath::new(dir_path)?;
