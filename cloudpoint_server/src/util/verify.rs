...
pub async fn run(db_pool: &PgPool) -> Result<Vec<(Uuid, Vec<String>)>> {
    ...
    for user_key in user_keys {
        ...
        sqlx::query("CREATE TEMP TABLE reachable (xxhash3_128 bytea PRIMARY KEY) ON COMMIT DROP")
            .execute(&mut *tx)
            .await?;

        let bytea: Vec<_> = cids.iter().map(|h| h.to_be_bytes().to_vec()).collect();

        sqlx::query("INSERT INTO reachable (xxhash3_128) SELECT * FROM UNNEST($1::bytea[])")
            .bind(&bytea)
            .execute(&mut *tx)
            .await?;

        let missing: Vec<Vec<u8>> = sqlx::query_scalar(
            "SELECT r.xxhash3_128 FROM reachable r
             WHERE NOT EXISTS (
                 SELECT 1 FROM chunks c
                 WHERE c.user_key = $1 AND c.xxhash3_128 = r.xxhash3_128
             )",
        )
        .bind(user_key)
        .fetch_all(&mut *tx)
