...
const CHUNK_GZ_DATA: &[u8] = &[
    0x1F, 0x8B, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0xFF, 0x63, 0x64, 0x60, 0x60, 0x00, 0x00,
    0x79, 0xB8, 0xF8, 0x99, 0x04, 0x00, 0x00, 0x00,
];
...
async fn chunk_head_ok(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        .oneshot(
            Request::head(format!("/sync/{USER_KEY}/chunks/xx/{CHUNK_CID:032x}"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn chunk_get_ok(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        .oneshot(
            Request::get(format!("/sync/{USER_KEY}/chunks/xx/{CHUNK_CID:032x}"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn chunk_put_ok(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        .clone()
        .oneshot(
            Request::put(format!("/sync/{USER_KEY}/chunks/xx/{CHUNK_CID:032x}"))
...
async fn chunk_put_malformed_errors(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        .clone()
        .oneshot(
            Request::put(format!("/sync/{USER_KEY}/chunks/xx/{:032x}", 012345_u128))
                .body(Body::from(CHUNK_GZ_DATA))
                .unwrap(),
        )
        .await
...
async fn version_get_latest_ok(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        .oneshot(
            Request::get(format!("/sync/{USER_KEY}/archives/{SYNC_ITEM}/?json"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    let file_entry = ver
        ...
        .unwrap();
    assert_eq!(
        u128::from_str_radix(file_entry.get("name").unwrap().as_str().unwrap(), 16).unwrap(),
        VERSION_CID
    );
    assert_eq!(
...
async fn version_get_latest_no_data(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        .oneshot(
            Request::get(format!("/sync/{USER_KEY}/archives/si_does_not_exist/?json"))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn version_get_ok(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        .oneshot(
            Request::get(format!(
                "/sync/{USER_KEY}/archives/{SYNC_ITEM}/{VERSION_CID:032x}"
            ))
            .body(Body::empty())
            .unwrap(),
        )
...
async fn version_get_no_access_not_found(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        .oneshot(
            Request::get(format!(
                "/sync/{}/archives/{SYNC_ITEM}/{VERSION_CID:032x}",
                Uuid::new_v4()
            ))
            .body(Body::empty())
            .unwrap(),
...
async fn ver_put_ok(db_pool: Pool<sqlx::Postgres>) {
    ...
    let response = app
        ...
        .oneshot(
            Request::put(format!(
                "/sync/{USER_KEY}/archives/{SYNC_ITEM}/{:032x}",
                ver.fingerprint()
            ))
